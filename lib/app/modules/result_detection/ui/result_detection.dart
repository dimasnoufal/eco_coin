...
class _ResultDetectionState extends State<ResultDetection> {
  ...
  Widget _buildLoadingImageClasification() {
    return Scaffold(
      appBar: AppBar(
  ...
  Widget _buildErrorImageClassification() {
  ...
  Widget _buildSuccessImageClassification(dynamic items) {
    Map<String, num> providerData = items as Map<String, num>;

    final confidencePercentage = (providerData.entries.first.value * 100)
        .toStringAsFixed(1);
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      providerData.entries.first.key.toString(),
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        'Kategori: ${providerData.entries.first.key.trim()}',
                                        style: AppColor.blackTextStyle.copyWith(
                                          fontSize: 16,
                                          fontWeight: FontWeight.w600,
                                        ),
                                      ...
                                      Text(
                                        _hintFromLabel(
                                          providerData.entries.first.key,
                                        ),
                                        style: AppColor.greyTextStyle.copyWith(
                                          fontSize: 12,
                                        ),
                                      ...
                                      child: Text.rich(
                                        TextSpan(
                                          ...
                                          children: [
                                            TextSpan(
                                              text:
                                                  '${_rewardFromLabel(providerData.entries.first.key)}',
                                              style: TextStyle(
                                                color: AppColor.emeraldDefault,
                                              ),
                                            ),
                            ...
                            if (_imagePath != null) {
                              ...
                              if (dbProvider.status) {
                                await Dialogs.showSuccessDialog(
                                  context: context,
                                  category: providerData.entries.first.key
                                      .toString(),
                                  ecoCoins:
                                      '${_rewardFromLabel(providerData.entries.first.key)}',
                                  onBackToHome: () {
    ...
    return Consumer<ImageClassificationProvider>(
      builder: (context, provider, _) {
        return switch (provider.resultState) {
          StaticResultLoading() => _buildLoadingImageClasification(),
          StaticResultLoaded(items: var items) =>
            _buildSuccessImageClassification(items),
          StaticResultError(message: var errorMessage) =>
            _buildErrorImageClassification(),
          StaticResultNone() => const SizedBox.shrink(),
        };
      },
    );
  }
